...
if __name__ == "__main__":
    ...
    # @jax.jit
    def get_action_and_value_pmap(
        agent_state: TrainState,
        next_obs: np.ndarray,
        key: jax.random.PRNGKey,
    ):
        next_obs = next_obs.reshape((NUM_DEVICES, -1) + next_obs.shape[1:])
        action, logprob, value, key = jax.pmap(get_action_and_value, in_axes=(None, 0, None), axis_name="batch")(agent_state, next_obs, key)
        return tuple(map(lambda x: np.array(x).reshape(-1, *x.shape[2:]), (action, logprob, value))) + (key[0],)
    ...
    def prepare_data(
        ...
        (last_env_ids, next_index_ranges), _ = jax.lax.scan(f, 
    ...
    def single_device_update(
        agent_state: TrainState,
        b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_inds,
        key: jax.random.PRNGKey,
    ):
        ...
        return agent_state, loss, pg_loss, v_loss, entropy_loss, approx_kl, b_inds, final_env_ids, key
    ...
    pmapped_update_step = jax.pmap(
        ...
        out_axes=(None, 0, 0, 0, 0, 0, 0, 0, None),
    )
    devices = jax.devices()
    def multi_device_update(
        agent_state: TrainState,
        b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_inds,
        key: jax.random.PRNGKey,
    ):
        # agent_state = jax.device_put_replicated(agent_state, devices)
        b_obs = jax.device_put_sharded(jnp.array_split(b_obs, len(devices)), devices)
        b_actions = jax.device_put_sharded(jnp.array_split(b_actions, len(devices)), devices)
        b_logprobs = jax.device_put_sharded(jnp.array_split(b_logprobs, len(devices)), devices)
        b_advantages = jax.device_put_sharded(jnp.array_split(b_advantages, len(devices)), devices)
        b_returns = jax.device_put_sharded(jnp.array_split(b_returns, len(devices)), devices)
        b_inds = jax.device_put_sharded(jnp.array_split(b_inds, len(devices)), devices)
        # key = jax.device_put_replicated(key, devices)
        return pmapped_update_step(agent_state, b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_inds, key)

    def multi_device_update2(
        agent_state: TrainState,
        b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_inds,
        key: jax.random.PRNGKey,
    ):
        agent_state = jax.device_put_replicated(agent_state, devices)
        b_obs = b_obs.reshape(len(devices), -1, *b_obs.shape[1:])
        b_actions = b_actions.reshape(len(devices), -1, *b_actions.shape[1:])
        b_logprobs = b_logprobs.reshape(len(devices), -1, *b_logprobs.shape[1:])
        b_advantages = b_advantages.reshape(len(devices), -1, *b_advantages.shape[1:])
        b_returns = b_returns.reshape(len(devices), -1, *b_returns.shape[1:])
        b_inds = b_inds.reshape(len(devices), -1, *b_inds.shape[1:])
        key = jax.device_put_replicated(key, devices)
        return pmapped_update_step(agent_state, b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_inds, key)

    def multi_device_update3(
        agent_state: TrainState,
        b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_inds,
        key: jax.random.PRNGKey,
    ):
        agent_state = jax.device_put_replicated(agent_state, devices)
        b_obs = jax.device_put_replicated(b_obs, devices)
        b_actions = jax.device_put_replicated(b_actions, devices)
        b_logprobs = jax.device_put_replicated(b_logprobs, devices)
        b_advantages = jax.device_put_replicated(b_advantages, devices)
        b_returns = jax.device_put_replicated(b_returns, devices)
        b_inds = jax.device_put_replicated(b_inds, devices)
        key = jax.device_put_replicated(key, devices)
        return pmapped_update_step(agent_state, b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_inds, key)
    ...
    for update in range(1, args.num_updates + 2):
        ...
        for step in range(async_update, (args.num_steps + 1) * async_update): # num_steps + 1 to get the states for value bootstrapping.
            ...
            # next_obs.reshape((2, -1) + next_obs.shape[1:]).shape
            # get_action_and_value(agent_state, next_obs.reshape((2, -1) + next_obs.shape[1:]), key)
            action, logprob, value, key = get_action_and_value(agent_state, next_obs, key)
            # action, logprob, value, key = get_action_and_value_pmap(agent_state, next_obs, key)
            # get_action_and_value_pmap(agent_state, next_obs.reshape((2, -1) + next_obs.shape[1:]), key)
        ...
        #     in_axes=(None, 0, 0, 0, 0, 0, 0, None),
        # )
        agent_state, loss, pg_loss, v_loss, entropy_loss, approx_kl, b_inds, final_env_ids, key = multi_device_update(agent_state, b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_inds, key)
        # raise
        # agent_state, loss, pg_loss, v_loss, entropy_loss, approx_kl, b_inds, final_env_ids, key = single_device_update(agent_state, b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_inds, key)
        # agent_state, loss, pg_loss, v_loss, entropy_loss, approx_kl, b_inds, final_env_ids, key = multi_device_update(agent_state, b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_inds, key)
        # %timeit single_device_update(agent_state, b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_inds, key)[1].block_until_ready()
        # %timeit multi_device_update(agent_state, b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_inds, key)[1].block_until_ready()
